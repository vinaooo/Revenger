...
/** Fragment for Load State slots grid. Allows user to select a slot to load a saved game state. */
class LoadSlotsFragment : SaveStateGridFragment() {
    ...
    override fun onSlotConfirmed(slot: SaveSlotData) {
        if (slot.isEmpty) {
            ...
            Toast.makeText(requireContext(), "Cannot load from empty slot", Toast.LENGTH_SHORT)
                    .show()
        } else {
    ...
    private fun showLoadConfirmationDialog(slot: SaveSlotData) {
        ...
        val dialog =
                AlertDialog.Builder(context)
                        ...
                        .setMessage(
                                "Load game state from ${slot.getDisplayName()}? Current progress will be lost."
                        )
                        .setPositiveButton("Load") { _, _ -> performLoad(slot) }
    ...
    private fun performLoad(slot: SaveSlotData) {
        try {
            ...
            if (success) {
                ...
                Toast.makeText(
                                requireContext(),
                                "Game loaded from ${slot.getDisplayName()}",
                                Toast.LENGTH_SHORT
                        )
                        .show()
                ...
                Toast.makeText(requireContext(), "Failed to load game state", Toast.LENGTH_SHORT)
                        .show()
            }
        } catch (e: Exception) {
