 ...
 * - etc.
 *
 * O modo é controlado externamente baseado no contexto (menu visível = MODO 1, jogo ativo = MODO
 * 2).
 */
class KeyboardInputAdapter(private var isInMenuMode: Boolean = true) : InputTranslator {
        ...
        val event =
                when (input.keyCode) {
                    ...
                    KeyEvent.KEYCODE_ENTER -> {
                        NavigationEvent.ActivateSelected(inputSource = InputSource.KEYBOARD)
                    }

                    // Voltar com Escape
                    KeyEvent.KEYCODE_ESCAPE -> {
                        NavigationEvent.NavigateBack(inputSource = InputSource.KEYBOARD)
                    }
     ...
     *
     * Quick jump permite ir direto para um item pressionando seu número. Útil para menus com muitos
     * itens.
     */
    private fun createQuickJumpEvent(index: Int): NavigationEvent {
        return NavigationEvent.SelectItem(index = index, inputSource = InputSource.KEYBOARD)
    }
    ...
    fun setMenuMode(menuMode: Boolean) {
        ...
        android.util.Log.d(TAG, "Keyboard mode: ${if (menuMode) "MENU" else "GAMEPLAY"}")
    }
}
