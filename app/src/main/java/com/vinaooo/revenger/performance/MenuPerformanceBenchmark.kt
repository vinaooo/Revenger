...
object MenuPerformanceBenchmark {
    ...
    private fun benchmarkAnimations(fragment: RetroMenu3Fragment): BenchmarkResult {
        ...
        // Measurement
        repeat(MEASUREMENT_ITERATIONS / 2) { // Menos iterações pois animações são mais lentas
            val startTime = SystemClock.elapsedRealtime()
            fragment.hideMainMenu()
    ...
    private fun logBenchmarkResults(results: MenuBenchmarkResults) {
        ...
        MenuLogger.performance("Menu Creation: ${results.menuCreationTime.averageTimeMs}ms (avg)")
        MenuLogger.performance("Menu Navigation: ${results.navigationTime.averageTimeMs}ms (avg)")
        MenuLogger.performance("Menu Animation: ${results.animationTime.averageTimeMs}ms (avg)")
    ...
    fun compareWithBaseline(
        ...
        val creationDiff =
                current.menuCreationTime.averageTimeMs - baseline.menuCreationTime.averageTimeMs
        comparison.append(
        ...
        // Comparar memória
        val memoryDiff =
                current.memoryUsage.allocatedMemoryKb - baseline.memoryUsage.allocatedMemoryKb
        comparison.append(
    ...
    fun benchmarkConfigurationComparison(): BenchmarkResult {
                ...
                MenuConfigurationBuilder.create()
                        ...
                        .addItem(
                                com.vinaooo.revenger.ui.retromenu3.MenuItem(
                                        ...
                                        "Item 1",
                                        action =
                                                com.vinaooo.revenger.ui.retromenu3.MenuAction
                                                        .CONTINUE
                                )
                        )
                        .addItem(
                                com.vinaooo.revenger.ui.retromenu3.MenuItem(
                                        "item2",
                                        "Item 2",
                                        action = com.vinaooo.revenger.ui.retromenu3.MenuAction.RESET
                                )
                        )
                        .addItem(
                                com.vinaooo.revenger.ui.retromenu3.MenuItem(
        ...
        // Warmup
        repeat(WARMUP_ITERATIONS) { RetroMenu3Fragment.newInstance(customConfig) }

        // Measurement
        repeat(MEASUREMENT_ITERATIONS) {
            val startTime = SystemClock.elapsedRealtime()
            RetroMenu3Fragment.newInstance(customConfig)
            val endTime = SystemClock.elapsedRealtime()
            times.add(endTime - startTime)
        }
