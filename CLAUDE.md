...
App icons: `python3 icons/scripts/master_icon.py` (also runs automatically as the `generateIcons` Gradle task before every build) scrapes console/game art from SteamGridDB/IGDB. It caches the online pick per ROM in `icons/.cache/auto_icon.png` + `auto_last_rom.txt` (gitignored), so later builds reuse it with no network; the cache is dropped when `rom` changes, and `./gradlew generateIcons -PrefreshIcons` (or `--refresh`, `--force N`, `--interactive`) scrapes again. Local fallbacks (console image, typography) are never cached. If every method fails the script exits 1 and the build fails. `./pick_icon.sh` opens an interactive HTML picker when the auto‑pick is bad; its choice is saved as an override (`custom_override_icon.png` + `last_rom.txt`) that beats the auto cache. Needs Python + Pillow (`venv/` is local and gitignored, not checked in). Runtime dependencies are declared in `icons/requirements.txt`. The repo `venv/` is broken (its `bin/python` and libs are different Python versions); use the system `python3` or a fresh venv.
