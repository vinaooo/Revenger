...
/**
 * Fila de eventos com debouncing para prevenir ações duplas e conflitos
 * entre múltiplas fontes de entrada.
 * 
 * O debouncing funciona ignorando eventos que chegam muito rápido após
 * outro evento (dentro da janela de debounce). Isso previne:
 ...
 */
class EventQueue(
    private val debounceWindowMs: Long = 200
) {
     ...
     */
    @Synchronized
    fun peek(): NavigationEvent? = queue.firstOrNull()
     ...
     * 
     * Útil quando queremos garantir que o próximo evento
     * não sofra debounce (ex: após abrir menu).
     */
    @Synchronized
