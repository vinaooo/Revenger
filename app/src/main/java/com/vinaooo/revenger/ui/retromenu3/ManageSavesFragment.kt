...
class ManageSavesFragment : SaveStateGridFragment() {
    ...
    override fun onSlotConfirmed(slot: SaveSlotData) {
        if (pendingOperation != null && sourceSlot != null) {
            ...
            if (slot.isEmpty) {
                Log.d("ManageSavesFragment", "Cannot manage empty slot")
                // TODO: Show toast
                return
            }
    ...
    private fun startOperation(operation: Operation, slot: SaveSlotData) {
        ...
        Log.d("ManageSavesFragment", "Started $operation from slot ${slot.slotNumber}")
    }
    ...
    private fun cancelOperation() {
        ...
        Log.d("ManageSavesFragment", "Operation cancelled")
    }
    ...
    private fun handleDestinationSelection(targetSlot: SaveSlotData) {
        ...
        if (targetSlot.slotNumber == source.slotNumber) {
            Log.d("ManageSavesFragment", "Cannot select same slot as destination")
            // TODO: Show toast
            return
        }
    ...
    private fun executeOperation(operation: Operation, sourceSlotNum: Int, targetSlotNum: Int) {
        ...
        if (success) {
            Log.d("ManageSavesFragment", "$operation successful: $sourceSlotNum -> $targetSlotNum")
        } else {
            Log.e("ManageSavesFragment", "$operation failed")
        }
    ...
    private fun confirmDelete(slot: SaveSlotData) {
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(R.string.dialog_delete) { _, _ ->
                ...
                if (success) {
                    Log.d("ManageSavesFragment", "Deleted slot ${slot.slotNumber}")
                }
                refreshGrid()
            }
    ...
    private fun showRenameDialog(slot: SaveSlotData) {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(R.string.dialog_rename) { _, _ ->
                ...
                if (success) {
                    Log.d("ManageSavesFragment", "Renamed slot ${slot.slotNumber} to '$newName'")
                }
                refreshGrid()
            }
            .setNegativeButton(R.string.dialog_cancel, null)
