...
App icons: `python3 icons/scripts/master_icon.py` (also runs automatically as the `generateIcons` Gradle task before every build) scrapes console/game art from SteamGridDB/IGDB. `./pick_icon.sh` opens an interactive HTML picker when the auto‑pick is bad. Needs Python + Pillow (`venv/` is local and gitignored, not checked in). The pure helpers in `icons/scripts/utils.py` have pytest tests in `icons/tests/`: `pip install -r icons/requirements-dev.txt`, then `python3 -m pytest icons/tests`. They are not part of `./gradlew check`, since Python and pytest may not be installed. They never read the real `icons/.env`.
